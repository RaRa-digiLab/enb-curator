...
#excludes missing languages |is.na(language)
...
p2 <- works[publication_date_cleaned< 2020&publication_date_cleaned>1750][language=="est",set:="Estonian"][language!="est"|is.na(language),set:="Other languages"] %>%
  ...
  #theme(legend.position = "none")
#p2
legend <- cowplot::get_plot_component(p2, 'guide-box', return_all = TRUE)[[3]]# + theme(legend.position = 'guide-box-bottom')

#cowplot::plot_grid(p1,p2, legend,ncol=1, rel_heights = c(4.5,4.5,0.5))
#ggsave("plots/blog1.png",cowplot::plot_grid(p1,p2, legend,ncol=1, rel_heights = c(4.5,4.5,0.5)),width=7,height=5,bg="white")
...
p01 <- works_l[,.N,.(language2,decade)][grid,on=c("language2","decade")][is.na(N),N:=0][decade>=1600&decade<2020][language2!=""][,prop:=N/sum(N),.(decade)][,lang_in_data:=sum(N),by=language2][,lang_w_n:=paste0(language2, " (n = ",lang_in_data,")")][,lang_w_n:=fct_reorder(lang_w_n,-lang_in_data)] %>%
  ...
  theme_bw()+
 # annotate("text",x=1600,y=1.3,label="Languages in Estonian National Bibliography (1600-2020)",color=cbbPalette[1],hjust=+0.05,size=5)+
  ...
  labs(y="Share of language (%)",x="Date of publication (y)")+#,subtitle=paste0("Keel (n = ",nrow(works2[!is.na(langauto5)]),")"))+ #title="Languages in Estonian National Bibliography (1600-2020)"
