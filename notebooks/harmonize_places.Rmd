...
places_rules <- fread("../config/placenames/placenames_rules.tsv",sep="\t")
...
all_places_updated <- merge(all_places,places_rules[order==2],by.x="place_for_edits",by.y="find_this",all.x=T)[is.na(replace_with),match:=place_for_edits][,.(place_unlist,place_for_edits=match)]
...
cities <- fread("../config/geoinfo/geonames_cities500.txt")
...
places_w_geonames_global[is.na(V1),uniqueN(place_for_edits)]
ee_places <- fread("../config/geoinfo/geonames_EE.txt")
...
places_w_arcgis <- fread("../config/geoinfo/places_arcgis_all.tsv",sep="\t")
...
tagged1 <- fread("../config/geoinfo/publication_place_geonames.tsv",sep="\t")[!is.na(lon)]
tagged2 <- fread("../config/geoinfo/publication_place_google.tsv",sep="\t")[!is.na(lon)]
...
fwrite(tartu_variants,"../reports/tartu_variants.tsv",sep="\t")
...
checked_places <- fread("../reports/sample_places_checked.tsv",sep="\t")
...
cluster_similarity_checked <- fread("../reports/cluster_similarity_checked.tsv",sep="\t")
...
rulebased_checked <- fread("../reports/rulebased_checked.tsv",sep="\t")
...
fwrite(both_methods2[order(groups)],"../reports/publishers_harmonize_both_methods_summary.tsv",sep="\t")
...
#
library(htmlwidgets)
saveWidget(p, file=paste0( getwd(), "../reports/networkInteractive.html"))
...
library(htmlwidgets)
library(plotly)
saveWidget(ggplotly(plot1), file=paste0( getwd(), "../reports/all_locations_on_map.html"))
