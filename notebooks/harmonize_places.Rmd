...
regexes_partial1 <- rules[type=="regex_partial"&order==1]
...
for (i in 1:nrow(regexes_partial1)){
  ...
  all_places[!is.na(replace_with),all_places:=replace_with]
  all_places[,all_places:=trimws(all_places)]
}


all_places_updated <- merge(all_places,places_rules[order==2],by.x="place_for_edits",by.y="place_for_edits",all.x=T)[is.na(match),match:=place_for_edits][,.(place_unlist,place_for_edits=match)]
