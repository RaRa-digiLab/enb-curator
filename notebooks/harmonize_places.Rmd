...
#compare with non-harmonized names
...
# unique(works4[,.(place_unlist)])
# 
# unique(works4[,.(harm_name,place_for_edits,place_unlist)])[,n:=.N,(place_unlist)][n>1]
# 
# harm_names <- unique(works4[,.(harm_name,place_for_edits,place_unlist)])
# harm_locations <- unique(works4_geo[!is.na(lat),.(harm_name,lat,lon)])
# 
# #est_preferred_where_possible[publication_place_control!="er"]
# authority_input_harm_names <- unique(harm_names[order(harm_name)][,.(place_original=place_unlist,place_harmonized=harm_name)])[!duplicated(place_original)]
# authority_input_harm_names
# 
# unique_name_locations <- merge(harm_names,harm_locations,by="harm_name")[,uniqueN(place_unlist)]
# 
# works4_geo[!is.na(lat),uniqueN(id)]/nrow(works)
# works4_geo[!is.na(lat),.(harm_name,lat,lon)]
# check <- works4_geo[is.na(lat),.(id,place_unlist,lat,lon)]
# no_names <- check[!str_detect(place_unlist,"[a-zA-ZÄÖÕÜõäöüîâīõ][a-zA-ZÄÖÕÜõäöüîâīõ]")&!str_detect(place_unlist,"\\p{Cyrillic}\\p{Cyrillic}"),uniqueN(id)]


maincomparison <- merge(unique(works4_geo_comp[,.(place_unlist,harm_name,lat_harm=lat,lon_harm=lon)])
,unique(works4_geo[,.(place_unlist,lat_raw=lat,lon_raw=lon)]),by="place_unlist")[!duplicated(place_unlist)]
