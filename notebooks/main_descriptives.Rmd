...
works[str_detect(edition_n,"^1 "),edition_statement:=NA]
...
values[,i:=.I]
values[,grouping:=c(rep("General",8),rep("Location",7),rep("Content",7),rep("Physical",5),rep("Access",4))]
...
values %>% 
    ggplot(aes(x = fct_reorder(variable,-i))) + #stats::reorder(variable, n_available)
    geom_hline(yintercept=311950,color="red",alpha=0.25,lty=2)+
    geom_bar(aes(y = n_available), stat = "identity", position = "dodge", width = 0.001, colour = "#484878", fill = "#484878") +
    geom_point(aes(y = n_available), colour = "#484878", fill = "#484878") + 
    coord_flip() + 
  facet_wrap(~fct_reorder(grouping,i),scales="free_y",ncol=1)+
    scale_color_discrete(guide = "none") + labs( y = "Data available", 
    x = "") + theme_minimal()+ ggforce::facet_row(vars(SP), scales = 'free', space = 'free')+ ggforce::facet_col(~fct_reorder(grouping,i), scales = 'free', space = 'free')# title= "Selected data fields in the curated dataset"

ggsave("plots/overview_data.png",height=7,width=6,bg="white")
